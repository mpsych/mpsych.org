...
function BibtexParser() {
    ...
    this.bibtex = function() {
        ...
        while (this.tryMatch("@")) {
            ...
            this.entries[this.currentEntry]["BIBTEXRAW"] = this.input.substring(start, end).replace('\\myname{','').replace('Daniel}', 'Daniel');
        }
    }
}
    ...
    this.displayAuthor = function(string, format) {
        ...
        if (format.find("span:not(a)").length) {
            for (i = 0; i < searchLength; i++) {
                ...
                // FIX FOR \myname
                if (arrayString[i].slice(0,7) == '\\myname') {
                    arrayString[i] = 'Haehn, Daniel';
                }
                ...
                if (fullName == 'Daniel Haehn') {
                    author.attr('class', 'danielhaehn');
                }
    ...
    this.createTemplate = function(entry, output) {
        ...
        for (var index in keys) {
            ...
            if (key == "AUTHOR") {
                ...
                // FIX FOR LONG AUTHORS
                if (smaller) {
                    value = "<span style='font-size:"+how+"em'>" + value + "</span>";                
                }
            } else if (key == "PAGES") {
                value = value.replace("--", "-");
            } else if (key == "DATE") {
